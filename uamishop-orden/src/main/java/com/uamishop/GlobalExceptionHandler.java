...
import com.uamishop.shared.exception.BusinessRuleException;
 ...
 *
 * @details
 *          Esta clase intercepta todas las excepciones lanzadas por los
 *          controladores
 *          (@RestController) y las transforma en respuestas HTTP estructuradas
 *          en formato JSON.
 *
...
public class GlobalExceptionHandler {
        ...
        // En uamishop-orden/src/main/java/com/uamishop/GlobalExceptionHandler.java

        @ExceptionHandler(DomainException.class)
        public ResponseEntity<ApiError> handleDomainException(DomainException ex, WebRequest request) {
                ApiError apiError = new ApiError(
                                HttpStatus.NOT_FOUND.value(),
                                "Not Found - Error de Dominio",
                                ex.getMessage(),
                                getPath(request));
                return new ResponseEntity<>(apiError, HttpStatus.NOT_FOUND);
        }
